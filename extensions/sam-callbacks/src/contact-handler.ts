...
function buildMergeKeyboard(taskId: string, state: Record<string, any>): Buttons {
  ...
  if (newData.email) {
    const selected = selections.email !== false;
    toggleRow.push({
      text: `${selected ? "\u2705" : "\u2b1c"} Email`,
  ...
  if (newData.phone) {
    const selected = selections.phone !== false;
    toggleRow.push({
      text: `${selected ? "\u2705" : "\u2b1c"} Phone`,
  ...
  if (action === "confirm") {
    ...
    try {
      ...
      if (selections.email !== false && newData.email) {
        mergeFields.push(`add_email=${JSON.stringify(newData.email)}`);
      }
      if (selections.phone !== false && newData.phone) {
        mergeFields.push(`add_phone=${JSON.stringify(newData.phone)}`);
      }
